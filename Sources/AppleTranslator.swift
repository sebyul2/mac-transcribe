...
final class AppleTranslator {
    /// Maps the LLM-style prompt language names (what the Translation tab
    /// stores) to the framework's language identifiers. "Auto" returns nil —
    /// the framework then detects the source itself.
    static func localeLanguage(forPrompt prompt: String) -> Locale.Language? {
        switch prompt {
        case TranslationLanguage.autoSource, "": return nil
        ...
        case "Traditional Chinese": return Locale.Language(identifier: "zh-Hant")
        ...
        let host = TranslatorHostView(
            ...
            onReady: { [weak self] ok in
                DispatchQueue.main.async {
                    ...
                    self.isReady = ok
                    if !ok { self.stop() }
                    SpeechService.diag("apple-translate ready=\(ok) \(source?.minimalIdentifier ?? "auto")->\(target.minimalIdentifier)")
                    readiness(ok)
                }
            }
        )
